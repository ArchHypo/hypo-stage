...
const SEED_STATEMENTS = [
  'Moving payment processing to a dedicated service will reduce coupling and allow independent scaling.',
  'Consolidating user identity in a single API will simplify auth and reduce duplicate logic across services.',
  'Replacing the legacy inventory system with a modern service will improve order fulfilment latency and reliability.',
  'Using an event-driven worker for notifications will decouple producers from delivery and improve retry behaviour.',
  'The example-website can stay as a static demo; no need to migrate to the new platform.',
  'Introducing a shared API gateway may centralise auth and rate limiting but adds a single point of failure.',
  'Migrating analytics to a dedicated data lake could improve reporting latency; impact on existing pipelines is low.',
];
...
exports.down = async function down(knex) {
  await knex('hypothesisEvents').whereIn(
    'hypothesisId',
    knex('hypothesis').select('id').whereIn('statement', SEED_STATEMENTS),
  ).del();
  await knex('hypothesis').whereIn('statement', SEED_STATEMENTS).del();
};
